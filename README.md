...
고객이 영화를 선택하고 좌석을 선택한다
고객이 결제한다 (status: 예매완료됨)
결제가 완료되면 예매 내역이 my page에 전달된다
영화관에서 나의 예매 정보로 티켓을 수령한다 (status: 티켓수령함)
고객이 예매를 취소할 수 있다 (티켓 수령 전)
예매가 취소되면 예매 정보가 사라진다
고객이 예매 내역을 중간중간 조회할 수 있다
비기능적 요구사항

2. 트랜잭션
결제가 되지 않은 예매 건은 아예 예매가 성립되지 않아야 한다 Sync 호출
장애격리
티켓 수령 기능이 수행되지 않더라도 예매는 365일 24시간 받을 수 있어야 한다 Async (event-driven), Eventual Consistency
결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다 Circuit breaker, fallback
성능
고객이 예매 내역을 my page(프론트엔드)에서 확인할 수 있어야 한다 CQRS
예매 상태가 바뀔때마다 카톡 등으로 알림을 줄 수 있어야 한다 Event driven
